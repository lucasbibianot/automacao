...
String PrintWithLeadingZero (unsigned long ReceiveUnsignedLong) {
  ...
  TempLz = ReceiveUnsignedLong & 0x0f000000; TempLz = TempLz >> 24; t2 = String(TempLz,HEX); //Serial.print(TempLz, HEX);
  
  TempLz = ReceiveUnsignedLong & 0x00f00000; TempLz = TempLz >> 20; t3 = String(TempLz,HEX); //Serial.print(TempLz, HEX);
  
  TempLz = ReceiveUnsignedLong & 0x000f0000; TempLz = TempLz >> 16; t4 = String(TempLz,HEX); //Serial.print(TempLz, HEX);
  
  TempLz = ReceiveUnsignedLong & 0x0000f000; TempLz = TempLz >> 12; t5 = String(TempLz,HEX); //Serial.print(TempLz, HEX);
  
  TempLz = ReceiveUnsignedLong & 0x00000f00; TempLz = TempLz >> 8; t6 = String(TempLz,HEX); //Serial.print(TempLz, HEX);
  
  TempLz = ReceiveUnsignedLong & 0x000000f0; TempLz = TempLz >> 4; t7 = String(TempLz,HEX); //Serial.print(TempLz, HEX);
  
  TempLz = ReceiveUnsignedLong & 0x0000000f; TempLz = TempLz >> 0; t8 = String(TempLz,HEX); //Serial.print(TempLz, HEX); 

  snprintf(msg, MSG_BUFFER_SIZE, "%s%s%s%s%s%s%s%s", t1, t2, t3, t4, t5, t6, t7, t8);
  return String(msg);
...
String Hash256(String InputString) {
  ...
  temp = temp + PrintWithLeadingZero(h0);
  temp = temp + PrintWithLeadingZero(h1);
  temp = temp + PrintWithLeadingZero(h2);
  temp = temp + PrintWithLeadingZero(h3);
  temp = temp + PrintWithLeadingZero(h4);
  temp = temp + PrintWithLeadingZero(h5);
  temp = temp + PrintWithLeadingZero(h6);
  temp = temp + PrintWithLeadingZero(h7);
