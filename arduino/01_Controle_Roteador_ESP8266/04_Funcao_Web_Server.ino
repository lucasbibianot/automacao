String processor(const String &var)
{
  ...
  else if (var == "CONECTADO") {
    return String(bPubConnectWeb ? "Sim" : "N&atilde;o");    
  }
  else if (var == "SITE_TESTE") {
    return String(config["site_check"]);    
  }
  return String();
}
...
void handleConfig(AsyncWebServerRequest * request) {
    ...
    config["temp"] = request->getParam("tempo_threshold", true)->value();
    ...
    config["site_check"] = request->getParam("input_site_check", true)->value();
...
void rotas_web_modo_1() {
    ...
    server.on("/json", HTTP_GET, [](AsyncWebServerRequest * request)
    {
        ...
        build_msg_json().toCharArray(msg, MSG_BUFFER_SIZE);   
    ...
    server.on("/get", HTTP_GET, [] (AsyncWebServerRequest * request) {
        ...
        if (request->hasParam(PARAM_INPUT_CONFIGTEMP) && request->hasParam(PARAM_INPUT_CONFIGSITE)) {
            inputMessage = request->getParam(PARAM_INPUT_CONFIGTEMP)->value();
            inputMessage2 = request->getParam(PARAM_INPUT_CONFIGSITE)->value();
            config["temp"] = inputMessage;
            config["site_check"] = inputMessage2;
            config["qtd_boot"] = "0";
            updateConfig(config);
        }
        else {
        ...
        request->send(200, "text/html", "HTTP GET request sent to your ESP on input field (" +
                                       String(PARAM_INPUT_CONFIGTEMP) + ") with value: " + inputMessage +
                                       "and input field (" + 
                                       String(PARAM_INPUT_CONFIGSITE) + ") with value: " + inputMessage2 +
